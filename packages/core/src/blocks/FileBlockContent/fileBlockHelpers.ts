...
export const createFileBlockWrapper = (
    ...
    editor: LcwDocEditor<BlockSchemaWithBlock<FileBlockConfig['type'], FileBlockConfig>, any, any>,
...
export const createDefaultFilePreview = (block: BlockFromConfig<FileBlockConfig, any, any>): { dom: HTMLElement; destroy?: () => void } => {
...
export const createFileAndCaptionWrapper = (block: BlockFromConfig<FileBlockConfig, any, any>, file: HTMLElement) => {
    ...
    if (typeof block.props.previewWidth === 'number' && block.props.previewWidth > 0 && block.props.caption !== undefined) {
...
export const createAddFileButton = (
    ...
    addFileButtonText.innerHTML = buttonText || editor.dictionary.file_blocks.file.add_button_text
...
export const parseFigureElement = (figureElement: HTMLElement, targetTag: string) => {
    const targetElement = figureElement.querySelector(targetTag) as HTMLElement | null
    if (!targetElement) {
        return undefined
    }
...
export const createLinkWithCaption = (element: HTMLElement, caption: string) => {
    const wrapper = document.createElement('div')
...
export const createFigureWithCaption = (element: HTMLElement, caption: string) => {
...
export const createResizeHandlesWrapper = (
    ...
    const windowMouseMoveHandler = (event: MouseEvent) => {
        if (!resizeParams) {
            if (!editor.isEditable && wrapper.contains(leftResizeHandle) && wrapper.contains(rightResizeHandle)) {
    ...
    const windowMouseUpHandler = (event: MouseEvent) => {
        if (
            (!event.target || !wrapper.contains(event.target as Node) || !editor.isEditable) &&
