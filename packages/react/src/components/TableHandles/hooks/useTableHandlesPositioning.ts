...
function getBoundingClientRectRow(
    ...
    if (draggingState && draggingState.draggedCellOrientation === 'row') {
        return new DOMRect(referencePosTable!.x, draggingState.mousePos, referencePosTable!.width, 0)
    }
...
function getBoundingClientRectCol(
    ...
    if (draggingState && draggingState.draggedCellOrientation === 'col') {
        return new DOMRect(draggingState.mousePos, referencePosTable!.y, 0, referencePosTable!.height)
    }
    ...
    useEffect(() => {
        ...
        refs.setReference({
            getBoundingClientRect: () => {
                const fn = orientation === 'row' ? getBoundingClientRectRow : getBoundingClientRectCol
