...
function CommentItem({
    ...
    return (
        <div className={`p-3 rounded-lg border ${isResolved ? 'bg-zinc-50 border-zinc-100' : 'bg-white border-zinc-200'}`}>
            {comment.anchorText && (
                <div className="mb-2 p-2 bg-zinc-100 rounded text-xs text-zinc-600 truncate">&ldquo;{comment.anchorText}&rdquo;</div>
            )}
            <p className={`text-sm ${isResolved ? 'text-zinc-500 line-through' : 'text-zinc-800'}`}>{comment.content}</p>
            <div className="flex items-center justify-between mt-2">
                <span className="text-xs text-zinc-400">{new Date(comment.createdAt).toLocaleDateString()}</span>
                <div className="flex items-center gap-1">
                    {!isResolved && (
                        <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => setIsReplying(!isReplying)}>
                ...
                <div className="mt-3 pl-3 border-l-2 border-zinc-200 space-y-2">
                    {comment.replies.map(reply => (
                        <div key={reply.commentId} className="text-sm group">
                            ...
                            <span className="text-xs text-zinc-400">{new Date(reply.createdAt).toLocaleDateString()}</span>
                        </div>
                    ))}
                </div>
            )}
        ...
        <div className="w-80 h-full border-l border-zinc-200 bg-white flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-zinc-200">
                <div className="flex items-center gap-2">
                    ...
                    {comments.length > 0 && <span className="text-xs text-zinc-500">({comments.length})</span>}
                </div>
                {onClose && (
                        ...
                        {unresolvedComments.length > 0 && (
                            <div className="space-y-3">
                                {unresolvedComments.map(comment => (
                                    <CommentItem key={comment.commentId} comment={comment} pageId={pageId!} onReply={handleReply} />
                                ))}
                            </div>
                        )}
            ...
            <div className="p-4 border-t border-zinc-200 space-y-3">
                ...
                />
                <Button className="w-full" onClick={handleCreateComment} disabled={!newComment.trim() || createMutation.isPending}>
