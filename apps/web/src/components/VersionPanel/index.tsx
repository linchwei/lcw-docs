...
export function VersionPanel({ pageId, onClose }: VersionPanelProps) {
    ...
    const rollbackMutation = useMutation({
        mutationFn: ({ pageId, versionId }: { pageId: string; versionId: string }) => srv.rollbackVersion(pageId, versionId),
        onSuccess: () => {
    ...
    const diffMutation = useMutation({
        mutationFn: ({ pageId, v1, v2 }: { pageId: string; v1: string; v2: string }) => srv.diffVersions(pageId, v1, v2),
        onSuccess: res => {
        ...
        <div className="w-80 h-full border-l border-zinc-200 bg-white flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-zinc-200">
                <div className="flex items-center gap-2">
                    ...
                    {versions.length > 0 && <span className="text-xs text-zinc-500">({versions.length})</span>}
                </div>
                {onClose && (
            ...
            <div className="p-4 border-b border-zinc-100">
                <Button className="w-full" variant="outline" onClick={handleCreate} disabled={createMutation.isPending}>
                    {createMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Plus size={16} className="mr-2" />}
                    保存当前版本
                </Button>
            </div>
                        ...
                        >
                            <div className="flex items-start justify-between">
                                <div className="flex-1 min-w-0">
                                    {version.description && (
                                        <p className="text-sm font-medium text-zinc-800 truncate">{version.description}</p>
                                    )}
                                    <div className="flex items-center gap-1.5 mt-1">
                                        <Clock size={12} className="text-zinc-400" />
                                        <span className="text-xs text-zinc-500">{formatDate(version.createdAt)}</span>
                        ...
                        {diffResult.added.length > 0 && (
                            <div>
                                ...
                                {diffResult.added.map((item, i) => (
                                    <p key={i} className="text-green-600 pl-2 truncate">
                                        + {item.content || item.blockType}
                                    </p>
                                ))}
                            </div>
                        )}
                        {diffResult.removed.length > 0 && (
                            <div>
                                ...
                                {diffResult.removed.map((item, i) => (
                                    <p key={i} className="text-red-600 pl-2 truncate">
                                        - {item.content || item.blockType}
                                    </p>
                                ))}
                            </div>
                ...
                <div className="p-4 border-t border-zinc-200 space-y-2">
                    ...
                    {confirmRollback ? (
                        <div className="space-y-2">
                            <p className="text-xs text-amber-600">确定要恢复到此版本吗？当前内容将自动保存为备份版本。</p>
                            <div className="flex gap-2">
                                ...
                                <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setConfirmRollback(false)}>
                                    取消
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" className="h-7 text-xs flex-1" onClick={() => setConfirmRollback(true)}>
                                <RotateCcw size={12} className="mr-1" />
                                恢复到此版本
                            </Button>
                            <Button
