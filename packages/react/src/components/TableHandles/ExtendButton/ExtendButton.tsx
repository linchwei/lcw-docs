...
function cropEmptyRowsOrColumns<I extends InlineContentSchema, S extends StyleSchema>(
    ...
    for (let i = content.rows.length - 1; i >= 0; i--) {
        if (removeEmpty === 'rows') {
            if (rows.length === 0 && content.rows[i].cells.every(cell => cell.length === 0)) {
                continue
            }
        }
...
const getContentWithAddedRows = <I extends InlineContentSchema, S extends StyleSchema>(
    content: PartialTableContent<I, S>,
    rowsToAdd: number,
    numCols: number
): PartialTableContent<I, S> => {
...
const getContentWithAddedCols = <I extends InlineContentSchema, S extends StyleSchema>(
    content: PartialTableContent<I, S>,
    colsToAdd: number
): PartialTableContent<I, S> => {
    ...
    return {
        type: 'tableContent',
        columnWidths: content.columnWidths ? [...content.columnWidths, ...newCells.map(() => undefined)] : undefined,
...
export const ExtendButton = <I extends InlineContentSchema = DefaultInlineContentSchema, S extends StyleSchema = DefaultStyleSchema>(
    ...
    useEffect(() => {
        const callback = (event: MouseEvent) => {
            ...
            const diff = (props.orientation === 'addOrRemoveColumns' ? event.clientX : event.clientY) - editingState.startPos
            ...
            const currentNumCells =
                props.orientation === 'addOrRemoveColumns' ? props.block.content.rows[0].cells.length : props.block.content.rows.length

            const newNumCells =
                numOriginalCells +
                marginRound(diff / (props.orientation === 'addOrRemoveColumns' ? EMPTY_CELL_WIDTH : EMPTY_CELL_HEIGHT), 0.3)

            if (newNumCells >= numCroppedCells && newNumCells > 0 && newNumCells !== currentNumCells) {
                props.editor.updateBlock(props.block, {
                    type: 'table',
                    content:
                        props.orientation === 'addOrRemoveColumns'
    ...
    return (
        <Components.TableHandle.ExtendButton
            className={mergeCSSClasses(
                'bn-extend-button',
                props.orientation === 'addOrRemoveColumns' ? 'bn-extend-button-add-remove-columns' : 'bn-extend-button-add-remove-rows',
