...
type ToolbarProps = ComponentProps['FormattingToolbar']['Root'] & ComponentProps['LinkToolbar']['Root']
...
export const ToolbarButton = forwardRef<HTMLButtonElement, ToolbarButtonProps>((props, ref) => {
    const { className, children, mainTooltip, secondaryTooltip, icon, isSelected, isDisabled, onClick, label, ...rest } = props
    ...
    const trigger =
        isSelected === undefined ? (
            ...
            <ShadCNComponents.Toggle.Toggle
                className={cn(className, 'bn:data-[state=open]:bg-accent bn:data-[state=closed]:text-accent-foreground')}
                aria-label={label}
                onClick={onClick}
                pressed={isSelected}
                disabled={isDisabled}
    ...
    return (
        <ShadCNComponents.Tooltip.Tooltip>
            <ShadCNComponents.Tooltip.TooltipTrigger asChild>{trigger}</ShadCNComponents.Tooltip.TooltipTrigger>
            <ShadCNComponents.Tooltip.TooltipContent className={'bn:flex bn:flex-col bn:items-center'}>
...
export const ToolbarSelect = forwardRef<HTMLDivElement, ComponentProps['FormattingToolbar']['Select']>((props, ref) => {
    ...
    return (
        <ShadCNComponents.Select.Select
            ...
            onValueChange={value => items.find(item => item.text === value)!.onClick?.()}
            disabled={isDisabled}
        >
            ...
            <ShadCNComponents.Select.SelectContent className={className} ref={ref}>
                {items.map(item => (
                    <ShadCNComponents.Select.SelectItem disabled={item.isDisabled} key={item.text} value={item.text}>
