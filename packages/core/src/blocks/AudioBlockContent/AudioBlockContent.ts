...
/**
 * 音频块渲染函数
 * 创建包含音频播放器和文件块包装器的 DOM 结构
 */
export const audioRender = (block: BlockFromConfig<typeof audioBlockConfig, any, any>, editor: LcwDocEditor<any, any, any>) => {
 ...
 * 音频块解析函数
 * 从 HTML 元素解析音频块的属性
 * 支持 audio 和 figure 标签
 */
export const audioParse = (element: HTMLElement): Partial<Props<typeof audioBlockConfig.propSchema>> | undefined => {
 ...
 * 用于导出或复制操作
 */
export const audioToExternalHTML = (block: BlockFromConfig<typeof audioBlockConfig, any, any>) => {
    if (!block.props.url) {
