...
export class CollaboratorController {
    ...
    @ApiBody({
        schema: {
            ...
            properties: {
                username: { type: 'string', description: '协作用户名' },
                role: { type: 'string', description: '协作角色', enum: ['editor', 'commenter', 'viewer'] },
            },
        },
    })
    @ApiResponse({
    ...
    async add(
        ...
        @Body(new ZodValidationPipe(addCollaboratorSchema)) body: AddCollaboratorDto,
        @Request() req
    ) {
    ...
    @ApiBody({
        schema: {
            type: 'object',
            required: ['role'],
            properties: { role: { type: 'string', description: '协作角色', enum: ['editor', 'commenter', 'viewer'] } },
        },
    })
    @ApiResponse({
        status: 200,
    ...
    async update(
        ...
        @Body(new ZodValidationPipe(updateCollaboratorSchema)) body: UpdateCollaboratorDto,
        @Request() req
    ) {
