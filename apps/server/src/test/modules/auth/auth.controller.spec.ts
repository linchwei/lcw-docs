...
describe('AuthController', () => {
    ...
    describe('POST /api/auth/logout', () => {
        it('AU-005: should logout when authenticated', async () => {
            const res = await request(app.getHttpServer()).post('/api/auth/logout').set('Authorization', `Bearer ${testUser.token}`)
            expect(res.status).toBe(201)
            expect(res.body.success).toBe(true)
        })
    ...
    describe('GET /api/currentUser', () => {
        ...
        it('AU-008: should return 401 when not authenticated', async () => {
            const res = await request(app.getHttpServer()).get('/api/currentUser')
            expect(res.status).toBe(401)
        })
    })
